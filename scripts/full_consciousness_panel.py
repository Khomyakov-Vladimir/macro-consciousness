#!/usr/bin/env python3
"""
# scripts/full_consciousness_panel.py

"Full Adaptive Consciousness Simulation with Weight Dynamics and Cluster-Macro Correlation"
...
# 2. Macro-consciousness predictive states
plt.figure(figsize=(10,5))
for k in range(K):
...
# 5. Inter-cluster weights dynamics
plt.figure(figsize=(12,6))
for i in range(M):
    for j in range(M):
        if i != j:
...
# 6. Spatial-temporal correlation between clusters and macro-projection
window = 20
    ...
    for k in range(K):
        for t in range(window, T):
            ...
            if np.std(cluster_window) > 1e-8 and np.std(macro_window) > 1e-8:
                ...
                corr_history[t, m, k] = 0.0

plt.figure(figsize=(12,6))
for m in range(M):
    for k in range(K):
        plt.plot(corr_history[:, m, k], label=f'Cluster {m} State {k}', alpha=0.7)
plt.xlabel('Time step')
plt.ylabel('Correlation with Macro Projection')
plt.title('Spatial-Temporal Correlation: Clusters vs Macro-Consciousness')
plt.grid(True)
plt.legend(loc='upper right', fontsize=8)
plt.show()
